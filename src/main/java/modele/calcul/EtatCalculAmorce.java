...
public class EtatCalculAmorce extends EtatCalcul{
    ...
    public EtatCalcul calculCourant(String operation) throws NonSupporteeException, OperationMalFormeeException {
        double resultatEnCours =
                this.getOperations().getResultat(this.operationPrecedente,operande1,this.getOperandeCourante());
        OperationHistorisee operationHistorisee = new OperationHistorisee(Double.toString(operande1),Double.toString(this.getOperandeCourante()), CalculatriceDynamiqueDuFuturImpl.reverseTraduction.get(this.operationPrecedente),Double.toString(resultatEnCours));
